...
**Status:** Physical QA complete; ImageBitmap and VideoFrame worker promotion rejected
...
**Status:** Complete; physical comparison rejects VideoFrame worker promotion
...
**Progress:** vendor `dcad73d` accepts and reports actual ImageBitmap/VideoFrame transfers while preserving one-request ownership and closure. CV `e4ef01b` adds a MediaPipe-only synchronous VideoFrame capture preset requiring exact rVFC media time, with deterministic timestamp/latest-replacement/retirement/unsupported tests. Assembly `ad485d1` filters and wires the explicit preset and exposes transfer type. Full touched-repo gates pass. The real Tasks 1.0.1 CPU/GPU desktop smoke is reproducible at assembly `e770627`/`c82a1e3` and recorded at CV `5fbafb9`: both accepted VideoFrame with 0 ms measured prep; this is compatibility proof, not phone performance evidence. Independent QA found that the option remained available without rVFC and that the cold smoke could race initial control configuration. Assembly `cdbbca4` now requires global VideoFrame plus HTMLVideoElement rVFC/cancel support, proves no-rVFC hiding in a real browser, and waits for the option before smoke selection; vendor `244c184` corrects the stale loader comment. Full gates pass, and HTTPS checkpoint `a656b57` exposes version `0.0.23` with the VideoFrame option only on the supported MediaPipe route. Independent targeted QA recheck passed: a live HTTPS no-rVFC probe hid and rejected the preset back to Direct-full, backend-switch filtering passed, the supported route exposed the preset, and the cold CPU/GPU smoke synchronization and loader wording were verified. A real-runtime synthetic-camera lifecycle smoke on `0.0.23` additionally passed CPU VideoFrame start, live worker→Direct-full retirement/restart, live CPU→GPU VideoFrame replacement with actual `webgl`, GPU worker→MoveNet reset, and browser disconnect without page errors; deterministic tests provide the terminal resource assertions. Round 5 supplied all four physical 120-sample Standard/Fast packets, summarized in `docs/telemetry/mediapipe-videoframe-mobile-moto-g-power-2026-08-27.md`. VideoFrame workers proved actual CPU-WASM/GPU-WebGL worker execution and 0ms preparation, improved callback p50 to 33ms and overlay cadence to 23fps, but regressed CPU total p50/p95 from 65/77ms to 80/115ms and GPU from 62/72ms to 69/86ms. Freshness regressed to 234ms CPU and 133ms GPU media-pose delta, with 8/10 safely retired stale transferables. VideoFrame promotion is rejected.
