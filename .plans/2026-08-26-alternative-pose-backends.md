...
**Status:** In Progress — focused contract/MoveNet gates passed/closed; ONNX terminal async-load fix `17d9d98` landed after vendor audit finding; vendor auditor rerun and selector/runtime gates remain
...
- Consolidated vendor QA initially passed MediaPipe through `678cb58` and ONNX through `c4c4da`, including provenance, CORS/weight exclusion, literal live/replay conformance, and official-model ONNX WASM smoke. Vendor audit then disproved ONNX terminal lifecycle under a deferred loader; fix `17d9d98` now shares concurrent load ownership, guards every async boundary, and releases late sessions once. Parent full gates and a real strict-WASM smoke passed after the fix (175.90ms wall load, 28.60ms wall zero-input estimate; non-phone evidence); auditor rerun is active.
